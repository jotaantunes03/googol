...
public class IndexStorageBarrel extends UnicastRemoteObject implements IndexStorageBarrelInterface {
    ...
    public List<String> searchWord(String words) throws RemoteException {
        ...
        // Ordenar os links
        List<String> sortedUrls = sortUrlsByLinkedCount(commonUrlsList);

        // Debug: imprimir links ordenados com numeração
        System.out.println("URLs retornadas ordenadas:");
        for (int i = 0; i < sortedUrls.size(); i++) {
            System.out.println((i + 1) + ": " + sortedUrls.get(i));
        }
