...
public class GoogolClient {
    ...
    public boolean addUrl(String url) {
        try {
            if (url == null || url.isEmpty()) {
                System.out.println("ERRO: URL não pode ser vazio!");
                return false;
            }

            if (!isValidUrl(url)) {
                System.out.println("ERRO: URL inválido!");
                return false;
            }

            gateway.addUrl(url);
            System.out.println("✅ URL enviado para indexação: " + url);
            return true;
        } catch (Exception e) {
            System.out.println("Erro ao indexar URL: " + e.getMessage());
            e.printStackTrace();
            return false;
        }
    }
    ...
    private void addUrl() {
        while (true) {
            ...
            boolean success = addUrl(userUrl); // Chama a nova versão
            if (success) {
                break;
            }
        }
