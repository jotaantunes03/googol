...
public class Gateway extends UnicastRemoteObject implements GatewayInterface, AutoCloseable {
    ...
    @Override
    public List<SearchResult> webSearch(String word) throws RemoteException {
        List<String> rawResults = search(word); // Reutiliza o método existente

        List<SearchResult> results = rawResults.stream()
                .map(raw -> {
                    String[] parts = raw.split("\n");
                    return new SearchResult(
                            parts[0].replace("URL", ""), // Extrai URL
                            parts[1],                    // Extrai título
                            parts.length > 2 ? parts[2] : "" // Extrai citação
                    );
                })
                .collect(Collectors.toList());
